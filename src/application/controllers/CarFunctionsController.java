...
public class CarFunctionsController implements Initializable {
	
	private static final Logger LOGGER = LoggerFactory.getLogger(CarFunctionsController.class);
	...
	public void sellCar(MouseEvent e) throws IOException {
		
		Car selectedCar = tableView.getSelectionModel().getSelectedItem();
		
		LOGGER.info("Selected car to sell {} {}", selectedCar.getMake(), selectedCar.getModel());
		
		if(null!=selectedCar) {
			
			FXMLLoader fxmlLoader = new FXMLLoader(getClass().getResource("/application/screens/SellCarScreen.fxml"));
		    Parent root1 = (Parent) fxmlLoader.load();
		    Stage stage2 = new Stage();
		    stage2.setUserData(selectedCar);
		    stage2.setScene(new Scene(root1, 600, 400));
		    SellCarController sellCarController = fxmlLoader.getController();
		    sellCarController.shareData(selectedCar);
		    stage2.show();
		}
